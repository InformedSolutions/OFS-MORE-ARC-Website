...
class AdultPreviousRegistrationDetailsForm(GOVUKForm):
    ...
    """
    error_summary_template_name = 'adult_update_templates/standard-error-summary.html'
    ...
    previous_registration = forms.ChoiceField(
        choices=choices,
    ...
    individual_id = forms.IntegerField(
        label='Individual ID:',
        widget=custom_number_input,
        required=True,
        error_messages={'required': "Enter individual ID"}
    )
    five_years_in_UK = forms.ChoiceField(
        choices=choices,
    ...
    def clean(self):
        ...
        if previous_registration == 'True' and individual_id is None:
