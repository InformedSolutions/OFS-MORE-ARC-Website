...
class FlaggingFunctionalityTests(NannyReviewFuncTestsBase):
    """
    This class tests the flagging functionality across all tasks.
    """

    ERROR_MESSAGE_BLANK_COMMENT = 'You must give reasons'

    def test_invalid_flag_first_aid_with_no_comment(self):
        fields_to_flag = [
            'training_organisation',
        ]

        post_data = self._create_post_data(fields_to_flag)
        post_data['training_organisation_comments'] = ''
        post_data['id'] = self.test_app_id

        response = self.client.post(reverse('nanny_first_aid_training_summary') + '?id=' + self.test_app_id, post_data)

        self.assertEqual(response.status_code, 200)

        self.assertInHTML(self.ERROR_MESSAGE_BLANK_COMMENT.encode(), response.content)
